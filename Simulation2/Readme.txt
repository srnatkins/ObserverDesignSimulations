This repository pertains to the simulations performed in Section 7.2.
The problem analyzed is of the form Eqn. (29).

Specific parameter settings within main2.m
A=[0 1; 0 0];
C = [1 0];
h=3.8;
tau1=0;
tau=0.125;
gap=11.9+20.25;  (this parameter is used to ensure that T_* is chosen to be > 2*(h+max(tau_i)+tau))
eps =1;
u0=[1;0;1;0];

NOTE: the simulations performed compare results with two different choices of \Delta(\xi,t) (those differences related to different sizes of Lipschitz constants). 
These simulations take a long time to run. For short run time simulations the subfolder SinglekD given within this folder may be better to look at. 


Some function handles constructed here 
main2.m     MAIN function handle that runs the problem and plots various figures. All of the functions below are called at some point when running the problem. 
...
NewAssumption5_1.m             
          ...
          %Lipschitz functions \Delta(\xi,t) with Lipschitz constants being smallkd and largekd respectively (where those constants specifically satisfy Assumption 5.1. 
          %INPUTS: 
          ...
          % T_^*>2(h+tau1+tau) hold as this is one condition needed for Assumption 2.2 
          %OUTPUTS:
...
%Based upon sim. 2 computations, betastar is a constant function so
%betastar is returned as number. Additionally, Lstar=1/betastar is a number 
%which exists so long as betastar is nonzero.  This ensures assumption 2 is
%satisfied

%kd>0 chosen such that kd*betabarnew<1 ensures that assumptioon 3 is
%satisfied. 
