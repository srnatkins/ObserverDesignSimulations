...
if tau1> tau+h  | tau1==h+tau %if tau1 satisfies condition described in remark of manuscript
    ...
    for i = 1: length(time)
        betastarvec(i) = C*(expm(tau*A)*ppval(L11,time(i)-tau1-tau))-C*E1*(ppval(L21,time(i)-tau1)-expm(-h*transpose(A))*ppval(L21,time(i)-tau1-h)); 
    end
   betastar=betastarvec;  
end
